...
export async function runTask() {
    ...
    await Promise.all(users
        ...
        try {
            ...
            try {
                await updateMonthifyPlaylists(user.spotify_user_id, access_token, is_new_month);
            }
            catch (error) {
                console.error(`Error updating playlists for user with display name '${user.spotify_display_name}':`, error);
            }
        }
        catch (error) {
            console.error(`Error retrieving access token for user with display name '${user.spotify_display_name}':`, error);
        }
    }));
}
// Run the task
