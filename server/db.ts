...
export async function updateCurrentMonth(current_month: number) {
  try {
    await pool.query(
      `
...
export async function getCurrentMonth() {
  try {
    const [rows] = await pool.query<RowDataPacket[]>(
      "SELECT current_month FROM month",
    );
    return rows[0].current_month;
  } catch (error) {
    console.error("Error getting value of current month from database:", error);
  }
}
